...
function MemberCard({ member, fetchMembers }) {
  ...
  useEffect(() => {
    async function fetchCellules() {
      if (!member.Ville) return; // majuscule
      const { data, error } = await supabase
        .from("cellules")
        .select("cellule, responsable, telephone")
        .eq("ville", member.Ville); // colonne dans table cellules
    ...
    <div className={`p-4 rounded-xl border shadow ${cardStyle}`}>
      ...
      <details className="mt-2">
        ...
        <div className="mt-2 text-sm text-gray-700 space-y-1">
          ...
          {/* Menu déroulant + WhatsApp seulement pour visiteur ou veut rejoindre ICC */}
            ...
            <div className="mt-2">
              ...
              >
                ...
                {cellules.length > 0
                  ...
                  : <option value="">Aucune cellule trouvée</option>
                }
              </select>
